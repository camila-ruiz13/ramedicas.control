...
export function CambioDonut({ counts }: { counts: CambioCount[] }) {
  if (counts.length === 0) {
    return (
  ...
  const data = counts.map((c) => ({
    label: CAMBIO_LABELS[c.cambio],
    value: c.count,
    fill: CAMBIO_COLORS[c.cambio],
  }));
  const config: ChartConfig = Object.fromEntries(
    counts.map((c) => [CAMBIO_LABELS[c.cambio], { label: CAMBIO_LABELS[c.cambio], color: CAMBIO_COLORS[c.cambio] }]),
  );
